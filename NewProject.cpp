...
void Read_Acc(ifstream &filein,ACC &x){
	getline(filein,x.user,',');
...
typedef struct NodeAcc *NODEACC;
struct ListAcc{
	NodeAcc *head;
	NodeAcc *tail;
};
typedef struct ListAcc LISTACC;
void InsertLastAcc(LISTACC &l,NODEACC x){
...
NODEACC createacc(ACC x){
	NODEACC newnode = new NodeAcc;
	newnode->data = x;
	newnode->next = NULL;
	return newnode;
}
void InitListAcc(LISTACC &l){
	l.head = NULL;
	l.tail = NULL;
}
void Read_All_Acc(ifstream &filein,LISTACC &l){
	while(!filein.fail()){
		ACC x;
		Read_Acc(filein,x);
		if (filein.fail())
        break;
		NODEACC newnode = createacc(x);
		InsertLastAcc(l,newnode);
	}
}
struct Date{
	int day;
...
bool login(string user,string pass){
	...
	Read_All_Acc(filein,l);
	NODEACC temp = l.head;
	while(temp!=NULL){
		if(LoaiBoXuongDongODau(temp->data.user)==user && temp->data.pass==pass ){
			return 1;break;
		}
		temp=temp->next;
	}
...
void OPTION(string user){
	...
	int option;
	ifstream filein;filein.open("danhsachbuukien.txt",ios::in);Doc_File(filein,l,ld);
